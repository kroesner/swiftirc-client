Attribute VB_Name = "modIOLEInPlaceActiveObjectTextInput"
Option Explicit
...
Public Type IPAOHookStructTextInput 'IOleInPlaceActiveObjectHook
    lpVTable As Long 'VTable pointer
    IPAOReal As IOleInPlaceActiveObject 'Un-AddRefed pointer for forwarding calls
    TBEx As ctlTextInput 'Un-AddRefed native class pointer for making Friend calls
    ThisPointer As Long
End Type
...
Public Property Get IPAOVTableTextInput() As Long
   ...
   End If
   IPAOVTableTextInput = m_lpIPAOVTable
End Property
...
Private Function AddRef(This As IPAOHookStructTextInput) As Long
   ' Call the UserControl's standard AddRef method:
   AddRef = This.IPAOReal.AddRef
End Function

Private Function Release(This As IPAOHookStructTextInput) As Long
   ' Call the UserControl's standard Release method:
   Release = This.IPAOReal.Release
End Function

Private Function QueryInterface(This As IPAOHookStructTextInput, riid As GUID, pvObj As Long) As Long
...
Private Function GetWindow(This As IPAOHookStructTextInput, phwnd As Long) As Long
   ' Call user controls' GetWindow method:
   GetWindow = This.IPAOReal.GetWindow(phwnd)
End Function

Private Function ContextSensitiveHelp(This As IPAOHookStructTextInput, ByVal fEnterMode As Long) As Long
   ' Call the user control's ContextSensitiveHelp method:
   ContextSensitiveHelp = This.IPAOReal.ContextSensitiveHelp(fEnterMode)
End Function

Private Function TranslateAccelerator(This As IPAOHookStructTextInput, lpMsg As VBOleGuids.Msg) As Long
Dim hRes As Long
...
Private Function OnFrameWindowActivate(This As IPAOHookStructTextInput, ByVal fActivate As Long) As Long
...
Private Function OnDocWindowActivate(This As IPAOHookStructTextInput, ByVal fActivate As Long) As Long
   ' Call the user control's OnDocWindow activate interface:
   OnDocWindowActivate = This.IPAOReal.OnDocWindowActivate(fActivate)
End Function

Private Function ResizeBorder(This As IPAOHookStructTextInput, prcBorder As RECT, ByVal puiWindow As IOleInPlaceUIWindow, ByVal fFrameWindow As Long) As Long
...
Private Function EnableModeless(This As IPAOHookStructTextInput, ByVal fEnable As Long) As Long
